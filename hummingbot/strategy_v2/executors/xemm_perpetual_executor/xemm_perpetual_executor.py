...
class XEMMPerpetualExecutor(ExecutorBase):
    ...
    def __init__(self, strategy, config: XEMMPerpetualExecutorConfig, update_interval: float = 1.0):
        super().__init__(
            strategy=strategy,
            connectors=[
    ...
    async def _set_leverage_and_position_mode(self):
        ...
        # Maker side
        maker_connector: DerivativeBase = self.connectors[self.config.maker_connector_pair.connector_name]
        maker_connector.set_leverage(
            self.config.maker_connector_pair.trading_pair,
            self.config.maker_leverage
        )
        maker_connector.set_position_mode(self.config.position_mode)
        
        # Taker side
        taker_connector: DerivativeBase = self.connectors[self.config.taker_connector_pair.connector_name]
        taker_connector.set_leverage(
            self.config.taker_connector_pair.trading_pair,
            self.config.taker_leverage
        )
        taker_connector.set_position_mode(self.config.position_mode)
        
        self.logger().info(
            f"Set leverage - Maker: {self.config.maker_leverage}x, "
            f"Taker: {self.config.taker_leverage}x, Mode: {self.config.position_mode.name}"
        )
    ...
    async def control_maker_order(self):
        ...
        if self._maker_order and self._maker_order.order:
    ...
    async def update_prices_and_tx_costs(self):
        ...
        self._maker_price = self.get_maker_order_price()
    ...
    async def is_profitable(self) -> bool:
        ...
        return net_profit >= self.config.min_profitability
    ...
    async def _check_single_side_margin(
        ...
        adjusted = self.adjust_order_candidates([order_candidate], connector_name)
    ...
    async def should_refresh_order(self) -> bool:
        ...
        # Check price change
        current_ideal_price = self.get_maker_order_price()
...
Please check the account and add margin if needed.
    ...
    async def validate_sufficient_balance(self):
        ...
        for connector_name, trading_pair, side, leverage in [
            ...
            adjusted = await self.adjust_order_candidates([order_candidate], connector_name)
